...
public class PullToRefreshListView extends ListView implements OnScrollListener
{
	...
	private void init(Context context)
	{
		...
		// 在构造函数内，是不能获取View的宽高的，需要先measure再获取,或者在ondraw内才可以
		measureView(headView);
		headContentHeight = headView.getMeasuredHeight();
		headContentWidth = headView.getMeasuredWidth();
		// 如果没有获取headview的高度，这里没法隐藏headerView.
	...
	{
		Log.d(this.getClass().getName(), "scrollState = " + scrollState);
	...
	private void measureView(View child)
	{
		...
		// 由于child(headView)还未加入到PullToRefreshListView中,所以p为null
		if (p == null)
		{
			p = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
		}
		// getChildMeasureSpec(int spec, int padding, int childDimension) 0 =
