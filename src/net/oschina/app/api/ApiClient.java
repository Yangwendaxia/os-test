...
public class ApiClient
{
	...
	private static String _MakeURL(String p_url, Map<String, Object> params)
	{
		...
		return url.toString().replace("?&", "?");// http://www.yonyou.com?name1=value1&name2=value2
	}
	...
	private static InputStream http_get(AppContext appContext, String url) throws AppException
	{
		...
		// 例:OSChina.NET/1.7.6.3_22/Android/4.0.4/MI-ONE
		// Plus/82258296-9899-452c-bbdc-43482e257284
	...
	private static InputStream _post(AppContext appContext, String url, Map<String, Object> params, Map<String, File> files) throws AppException
	{
		...
		do
		{
			try
			{
				...
				{
					Cookie[] cookies = httpClient.getState().getCookies();// [oscid=PR7FwfDo%2F%2BLgKYItYNcGB4iXdL2F8MQgfSf7kVY4H3xQVpuay59OfmoGsblVWtI%2B%2FBG8Sex7LbvFM4gnMCOGEoGLu1opJB2p%2B4jYItCrMnIes%2B4%2BdT%2FJAusIy9RU4DtH]
					String tmpcookies = "";
		 ...
		 * 
		 * 
		 * <oschina> <result> <errorCode>1</errorCode>
		 * <errorMessage><![CDATA[登录成功]]></errorMessage> </result> <user>
		 * <uid>248884</uid> <location><![CDATA[江西 南昌]]></location>
		 * <name><![CDATA[yangwendaxia]]></name> <followers>0</followers>
		 * <fans>0</fans> <score>0</score> <portrait></portrait> </user>
		 * <notice> <atmeCount>0</atmeCount> <msgCount>0</msgCount>
		 * <reviewCount>0</reviewCount> <newFansCount>0</newFansCount> </notice>
		 * </oschina> <!-- Generated by OsChina.NET
		 * (init:1[ms],page:2[ms],ip:118.212.199.60) -->
		 */
		// 这段代码做什么用?每次网络通讯都自带Result结果解析,主要处理用户未登录的异常情况，此时需要显示登录对话框
		if (responseBody.contains("result") && responseBody.contains("errorCode") && appContext.containsProperty("user.uid"))
		{
			try
			{
				Result res = Result.parse(new ByteArrayInputStream(responseBody.getBytes()));
				if (res.getErrorCode() == 0)// 用户未登录
				{
					appContext.Logout();
					appContext.getUnLoginHandler().sendEmptyMessage(1);
				}
	...
	public static User login(AppContext appContext, String username, String pwd) throws AppException
	{
		...
		String loginurl = URLs.LOGIN_VALIDATE_HTTP;// http://www.oschina.net/action/api/login_validate
		if (appContext.isHttpsLogin())
		{
			loginurl = URLs.LOGIN_VALIDATE_HTTPS;// https://www.oschina.net/action/api/login_validate
		}
		try
		{
